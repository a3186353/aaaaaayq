...
int luaopen_mygxy_jy_depth(lua_State* L);
int luaopen_mygxy_asset(lua_State* L);
int luaopen_mygxy_thx_async(lua_State* L);
MYGXY_API int luaopen_mygxy(lua_State* L)
{
    ...
    lua_pushcfunction(L, luaopen_mygxy_jy_depth);
    lua_call(L, 0, 1);
    lua_setfield(L, -2, "JyDepth");

    lua_pushcfunction(L, luaopen_mygxy_asset);
    lua_call(L, 0, 1);
    lua_setfield(L, -2, "Asset");

    lua_pushcfunction(L, luaopen_mygxy_thx_async);
    lua_call(L, 0, 1);
    lua_setfield(L, -2, "Thx");
